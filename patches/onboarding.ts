...
/**
 * 🌟 直接发起 Google OAuth 授权跳转（无需展示中间过渡页面）
 */
export function startGoogleDirectLogin(res: ServerResponse): void {
  pruneCliSessions();
  const { verifier, challenge } = generatePkce();
  const oauthState = generateState();
...
/**
 * 🌟 51121 端口自动捕获 Google OAuth 回调并自动入库
 */
export async function handleAutoOAuthCallback(
  ...
  if (!verifier && pendingSessions.has(state)) {
    verifier = pendingSessions.get(state)!.verifier;
    pendingSessions.delete(state);
  }
  ...
  if (!verifier) {
    return {
      ok: false,
      error: "未找到匹配的授权会话或会话已超时，请返回仪表盘重新发起登录。",
    };
  }

  if (matchedSessionId) {
    cliLoginSessions.delete(matchedSessionId);
  }

  try {
    const tokenData = await exchangeAuthorizationCode(code, verifier);
    const email = await getUserEmail(tokenData.accessToken);
    const project = await discoverProject(tokenData.accessToken);
    const label = email ? email.split("@")[0] : "Account";
    const entry = {
    ...
    await addAccountToConfig(entry);
    await rotator.addOrUpdateAccount(entry);
    return { ok: true, email: entry.email };
  } catch (err: any) {
    return { ok: false, error: err.message || "OAuth 凭据交换失败" };
  }
}
